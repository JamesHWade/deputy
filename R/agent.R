...
Agent <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(
      ...
      agent_id <- validate_session_id(
        agent_id %||% paste0("agent_", generate_session_id()),
        argument = "agent_id"
      )
      if (
        !is.null(agent_name) &&
    ...
    #' - Working directory
    #' - Constructor defaults and the latest effective run context
    ...
    #'
    #' @details
    #' Hooks are NOT restored from sessions as they contain function closures
    #' that may not serialize correctly. Constructor permissions and
    #' `working_dir` always remain authoritative. Saved run context is validated
    #' before conversation state changes, then merged with constructor context;
    #' protected identity conflicts fail the load.
  ...
  private = list(
    ...
    build_session_payload = function(extra_metadata = list()) {
      ...
      list(
        ...
        run_context_defaults = clone_run_context(private$.run_context),
    ...
    restore_session_payload = function(
      ...
      restored_run_context <- tryCatch(
        {
          saved_defaults <- normalize_run_context(
            session$run_context_defaults %||% list(),
            argument = "session$run_context_defaults"
          )
          saved_effective <- normalize_run_context(
            session$run_context %||% saved_defaults,
            argument = "session$run_context"
          )
          merge_run_context(
            merge_run_context(private$.run_context, saved_defaults),
            saved_effective
          )
        },
    ...
    hook_context = function(..., delegation_id = NULL) {
      ...
      if (!is.null(private$compat_session_id)) {
        context$session_id <- private$compat_session_id
      }
      context
    },
    ...
    tool_call_record = function(extracted, phase) {
      ...
      if (
        is.null(tool_use_id) ||
          ...
          is.na(tool_use_id) ||
          !nzchar(as.character(tool_use_id))
      ) {
        tool_use_id <- NULL
      } else {
        tool_use_id <- as.character(tool_use_id)
      }
      ...
      if (!is.null(tool_use_id)) {
        matches <- which(vapply(
          records,
          function(record) identical(record$tool_use_id, tool_use_id),
          logical(1)
        ))
      ...
      if (is.null(index)) {
        ...
        records[[index]] <- list(
          tool_call_id = tool_use_id %||%
            paste0("tool_", generate_session_id()),
          tool_use_id = tool_use_id,
          tool_name = tool_name,
          delegation_id = if (identical(tool_name, "delegate_to_agent")) {
            paste0("delegation_", generate_session_id())
          } else {
            NULL
          },
    ...
    claim_delegation = function() {
      ...
      list(
        tool_call_id = NULL,
        delegation_id = paste0("delegation_", generate_session_id()),
        parent_agent_id = private$.agent_id,
        parent_run_id = private$active_run_id(),
        run_context = private$effective_run_context()
      )
      ...
      if (!is.null(private$.file_checkpoints)) {
        tryCatch(
          private$.file_checkpoints$after_tool(
            extracted$tool_use_id %||% record$tool_call_id,
