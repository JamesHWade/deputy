...
Agent <- R6::R6Class(
 ...
 public = list(
   ...
   provider = function() {
     ...
     name <- tryCatch(provider@name, error = function(e) provider$name %||% "unknown")
     model <- tryCatch(provider@model, error = function(e) provider$model %||% "unknown")
     list(
       name = name,
       model = model
     )
   ...
   load_session = function(path, restore_tools = TRUE) {
     ...
     required_fields <- c("turns", "system_prompt", "permissions", "working_dir")
     ...
     if (restore_tools && !is.null(session$tools) && length(session$tools) > 0) {
       tryCatch({
     ...
     } else if (!is.null(session$tool_names) && length(session$tool_names) > 0) {
       cli_warn(c(
     ...
     if (!is.null(session$loaded_skills) && length(session$loaded_skills) > 0) {
   ...
   compact = function(keep_last = 4, summary = NULL) {
     ...
     if (length(turns) <= keep_last) {
       cli_alert_info("Not enough turns to compact (have {length(turns)}, keep_last = {keep_last})")
       return(invisible(self))
     }
     ...
     if (is.null(summary)) {
       ...
       summary_parts <- vapply(turns_to_compact, function(turn) {
       ...
       summary <- paste0(
         "[Compacted ", compact_count, " earlier turns]\n\n",
     ...
     cli_alert_success("Compacted {compact_count} turns, keeping {length(turns_to_keep)}")
     invisible(self)
   },
 ...
 private = list(
   ...
   on_tool_result = function(result) {
     ...
     tool_name <- if (!is.null(result@request)) result@request@name else "unknown"
     ...
     coro::generator(function() {
       ...
       for (i in seq_len(max_turns)) {
         ...
         if (agent$.__enclos_env__$private$should_stop) {
           stop_reason <- agent$.__enclos_env__$private$stop_reason_from_hook %||% "hook_requested_stop"
           break
         }
         ...
         if (!is.null(agent$permissions$max_cost_usd)) {
           ...
           if (!is.na(current_cost) && current_cost >= agent$permissions$max_cost_usd) {
             stop_reason <- "cost_limit"
             break
           }
           if (!is.na(current_cost) && current_cost >= agent$permissions$max_cost_usd * 0.9) {
             cli::cli_warn("Approaching cost limit: {format_cost(current_cost)} / {format_cost(agent$permissions$max_cost_usd)}")
           }
         }

         # Determine the prompt for this turn
         prompt <- if (i == 1) task else NULL
         ...
         if (!is.null(stream_gen)) {
           ...
           repeat {
             ...
             if (!is.null(chunk) && nchar(chunk) > 0) {
               ...
               coro::yield(AgentEvent("text", text = chunk, is_complete = FALSE))
             }
           }
         } else {
           ...
           if (!is.null(response) && nchar(response) > 0) {
             ...
             coro::yield(AgentEvent("text", text = response, is_complete = TRUE))
           }
         }
         ...
         # Stall detection
         current_hash <- if (nchar(full_text) > 0) digest::digest(full_text, algo = "md5") else ""
         if (!is.null(last_response_hash) && identical(current_hash, last_response_hash) && nchar(full_text) > 0) {
         ...
         coro::yield(AgentEvent("turn", turn = last_turn, turn_number = turn_num))
