...
permissions, hooks, and streaming output. It also ships an Anthropic-compatible
Claude Agent SDK facade for teams that want Claude-style entrypoints, tool
aliases, settings, and persisted session workflows.

> **Note:** deputy keeps its R-native `Agent` and `LeadAgent` APIs as the
> canonical runtime. The Claude-compatible layer is opt-in via
...
`WebFetch`, `WebSearch`, and `Task` (when sub-agents are registered).
