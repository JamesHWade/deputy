...
`agent$stream()` in synchronous code, or pass `agent$stream_async()` directly
to `shinychat::chat_append()`. Those paths use the same governed kernel as
`run_sync()` and `run_async()`; the latter pair return richer `AgentResult`
metadata when the host needs to inspect the run.
