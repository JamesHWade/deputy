...
quickstart_chat <- docs$create_chat(list(
  docs$step_tool(
    "I'll inspect the fixture project.",
    "list_files",
    list(path = ".")
  ),
  docs$step_text(
...
stream_chat <- docs$create_chat(list(
  docs$step_text(
    ...
    chunks = c("Streaming ", "example ", "ready.")
  )
))
```
...
```{r example, eval = docs$should_eval}
...
Tool policy settings from `settings.json` are also applied to `agent$permissions`
(`allowedTools`, `disallowedTools`, and `permissionPromptToolName`).
...
```{r structured_output, eval = FALSE}
schema <- list(
  type = "object",
