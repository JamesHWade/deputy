...
This example assigns separate reviewers to bugs, code style, and
documentation in an R package. A lead agent combines their findings into
a report. Each reviewer has its own prompt and conversation, so you can
inspect its results separately.
...
# Never substitute zero for an incomplete provider total
if (!isTRUE(result$cost$complete)) {
  cli::cli_alert_warning(
    "Provider omitted {result$cost$missing} cost record{?s}."
  )
}
