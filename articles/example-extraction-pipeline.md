...
This example builds a pipeline that extracts structured metadata from an
...
Check `is_success()` before passing a result to the next step. Use
`validate` for application rules; invalid output or an unavailable
validator signals an error, and bounded corrections are opt-in:
...
- [`vignette("permissions")`](https://jameshwade.github.io/deputy/articles/permissions.md)
  – Permission presets and read-only mode
- [`vignette("example-code-review")`](https://jameshwade.github.io/deputy/articles/example-code-review.md)
  – Multi-agent orchestration with LeadAgent
