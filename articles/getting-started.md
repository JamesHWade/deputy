...
examples use OpenAI’s `gpt-5.6-luna` with `OPENAI_API_KEY`, but the
Deputy API is provider-independent. To use a larger GPT-5.6 model,
select `gpt-5.6-terra` or `gpt-5.6-sol` explicitly when creating the
Chat. The `gpt-5.6` alias selects Sol.
