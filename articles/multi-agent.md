...
For complex tasks, deputy supports a **lead agent** that delegates work
to specialised **sub-agents**. Each sub-agent has its own tools, prompt,
and (optionally) model. The lead agent decides when to delegate and
...
- Different parts of a task need different tools (e.g., code analysis
  vs. data analysis)
...
For simpler tasks, a single `Agent` with the right tools is usually
enough.

## Composing retained specialists

For existing, independently configured conversations, use an ordinary
`Agent` as the root and `retain_agent_graph()` to declare the permitted
routes. The following assumes `root`, `analyst` and `reviewer` are
distinct Agents whose permissions already allow the relevant tools:
...
The root owns all retained conversations and descendant records. Each
route fixes its target and allocation; the model supplies only a new
task brief. Provider configuration, prompts and executable tools stay
with each specialist. Policies and tool hooks from every ancestor also
govern descendant tool calls. A child cannot grant itself more authority
than its callers.

The graph budget is cumulative across root runs and explicit host
follow-ups. Depth counts edges from the root. Concurrency includes
parents waiting for their children, so this three-level route requires
two child slots. At capacity calls reject before dispatch. Cyclic routes
can be configured, but a busy conversation cannot be reentered. Token
and cost ceilings use reported responses; requests already in flight can
overshoot them. A request ceiling bounds dispatch itself.
...
Observation and selected-child panels never consume a producer stream or
inject child transcripts into the root’s model context. Cancellation
cooperatively stops a subtree; siblings remain independently
inspectable. Release removes graph routes and ownership, while borrowed
tools and external resources remain the host’s responsibility. These
handles do not survive an R restart.

The `examples/recursive-agents` Shiny app provides a deterministic local
HTTP producer and separate descendant transcript selection, with no paid
model calls. Each session owns a separate graph and its host controls.
The example’s requester token scopes disclosure; shared applications
...
internally. The sub-agent runs to completion and returns its result to
the lead:
...
child inherits the lead’s remaining `UsageLimits`, and its usage is
aggregated into `result$usage` and enforced by the lead after
delegation. Concurrent direct children reserve allocations before
launch. Background children, transitive child trees, and cross-run
global limits remain outside this contract.

## Stateless fan-out

For independent perspectives on supplied text, the host can select
responders directly. Each definition gets a fresh conversation with its
own prompt and at most one model request. The lead makes no model
request for orchestration. Definitions with tools, skills, or MCP
servers are rejected before dispatch.
...
Names are normalized and must select each registered definition at most
once. The result preserves input order and has five fields:
...
A failed responder does not discard successful siblings. Check `status`
before synthesis; a stopped child may have partial text. Child records
include unique agent, session, run, and delegation identifiers linked to
the batch run, and fire the usual `SubagentStart` and `SubagentStop`
hooks. `$last_run()` retains the aggregate on normal return. The lead’s
conversation is unchanged; the host explicitly supplies selected
responses to any later synthesis run.

### Limits and cancellation

`max_active` bounds simultaneous responders. Work runs in waves: the
next wave starts after all current responders settle. Each wave reserves
request slots before dispatch and divides the remaining token and cost
ceilings among its responders. A definition can narrow its request
allowance to zero. Failed dispatch attempts count as requests; missing
cost information stops queued work when a cost ceiling is configured.
...
interrupted runs. The default `on_exceed = "stop"` returns partial
outcomes. `on_exceed = "error"` raises the usual typed limit condition
after cleanup; completed child records remain inspectable through
`$list_subagents()` and `$get_subagent_results()`.

For Shiny or another asynchronous host, use `$parallel_delegate_async()`
and consume its promise. `$interrupt()` prevents queued work from
starting and asks active responders to stop at the next supported
provider boundary. The batch settles after its active responders drain.
The lead rejects overlapping runs until cleanup finishes. Tool-using
background workers and persistent agent scheduling are separate future
work.

## Opposing perspectives and synthesis

For a complete opposing-perspective workflow, run the installed
standalone example after configuring your OpenAI credentials:
...
It prints a Markdown comparison, requires both heads to complete, and
then uses a separate one-request moderator. Its bundled prompt can also
be reused with any Agent:
...
Loading this prompt does not start independent responders. The host
supplies arguments or orchestrates fan-out explicitly.

## Inspecting child conversations

`lead$list_subagents()` has one row per admitted delegation in admission
order, even while children are running. `delegation_id` remains stable
as `status` changes from `queued` to `running` and then a settled
outcome:
...
`stop_reason` preserves the exact runtime reason. Completion does not
verify the assigned task or its evidence. `admitted_at` records
acceptance, `started_at` records lifecycle entry before the start hook,
and `completed_at` records the invocation settling, including
suspension. Cancellation in a start hook can therefore yield
`not_started` with a start timestamp. Run IDs are unavailable until the
child starts a run. Callback failures follow the existing hook policy
...
token stream. Reading them does not copy them into the lead’s context,
resume a child, or approve a pending action. The host must authorize and
redact any user-facing disclosure. These records belong to the live
LeadAgent; they are not persisted as a durable child job store.

For an asynchronous lead run,
`lead$interrupt(reason = "cancelled_by_host")` cooperatively interrupts
...
The explicit handoff is necessary because delegated child approval
continuation is not yet supported. The receipt keeps the source child
delegation separate from the execution Agent. Reading a child remains
