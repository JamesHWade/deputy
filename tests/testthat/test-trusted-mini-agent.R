...
test_that("trusted study computes only after review and keeps model prose separate", {
  ...
  trusted <- result_trusted_results(result$commentary, "study_summary")
  expect_length(trusted, 1L)
  expect_identical(trusted[[1L]]$tool_name, "compute_summary")
  expect_identical(trusted[[1L]]$arguments$plan, x$plan)
