...
vignette: >
  %\VignetteIndexEntry{Agent Configuration}
...
docs <- docs_init("agent-configuration", "verified-replay")
project <- docs$make_project()

setwd(project$root)
knitr::opts_knit$set(root.dir = project$root)

agent_result_chat <- docs$create_chat(list(
  docs$step_tool(
    "I'll check the fixture project first.",
    "list_files",
    list(path = ".")
  ),
  docs$step_text("The fixture project contains a DESCRIPTION file, one R script, and one CSV file.")
))
```
...
This vignette covers the many ways to configure an agent: system prompts,
settings, skills, sessions, and result inspection.

## System Prompts

Every agent has a system prompt that shapes its behaviour. Pass it directly
at construction:
...
The system prompt is prepended to the conversation. If you also load skills
(see below), their prompts are appended after the system prompt.
...
Skills are modular extensions that add tools and system prompt segments to an
agent. They live in directories with a `SKILL.yaml` (or `SKILL.md`) file.
...
Sessions persist the conversation turns, so the agent can pick up where it
left off.
...
```{r agent-result-demo, eval = docs$should_eval}
chat <- agent_result_chat
