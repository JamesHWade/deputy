...
**Note:** We set `timeout = 0` so hooks run in the main R process. By
default, hooks run in a callr subprocess (for safety), which means `<<-`
assignments can't reach the parent environment. Use `timeout = 0` when your
hook needs to modify external state:
