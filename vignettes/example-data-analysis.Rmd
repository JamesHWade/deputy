...
The agent can read data and run R code, but cannot write files or run bash
commands. This permission is application policy, not an OS sandbox. Deputy
stops the run when it observes either configured usage limit, and stops with
`"cost_unavailable"` if the provider does not report enough cost information to
enforce the dollar limit.

## Next Steps

- `vignette("tools")` -- Tool bundles and custom tools
- `vignette("getting-started")` -- Skills, sessions, and run results
