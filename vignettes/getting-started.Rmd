...
Results are read-only S7 values. For example, `S7::prop(result, "response")`
reads the same value as `result$response`. Construct a separate `AgentResult()`
when you need a different record; changing a completed result's properties is
not supported. Original provider objects and conditions retain their own
reference semantics inside the record.
