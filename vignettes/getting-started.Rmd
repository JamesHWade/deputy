...
docs <- docs_init("getting-started", "verified-replay")
...
quickstart_chat <- docs$create_chat(list(
  docs$step_tool(
    "I'll inspect the working directory first.",
    "list_files",
    list(path = ".")
  ),
  docs$step_text(
    "The project contains DESCRIPTION, R/hello.R, data/people.csv, and notes/intro.md."
  )
))

streaming_chat <- docs$create_chat(list(
  docs$step_text(
    "deputy streams text and events as it works.",
    chunks = c("deputy ", "streams text ", "and events as it works.")
  )
))
```
...
```{r getting-started-quickstart, eval = docs$should_eval}
agent <- Agent$new(chat = quickstart_chat, tools = tools_file())
...
```{r getting-started-streaming, eval = docs$should_eval}
agent <- Agent$new(chat = streaming_chat, tools = tools_file())
