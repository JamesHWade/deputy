...
Hooks are S7 values created with `HookMatcher()`. Their configuration is
read-only after construction. Use `S7::prop(hook, "event")` to read a property
and `hook_matches(hook, "write_file")` to test a tool name. Callback closures
retain their caller-owned state.
...
Hook result constructors return read-only S7 values. Inspect properties with
`@`, `$`, or `S7::prop()` and use `S7::props()` for a plain record. To change a
decision, construct a new result. Nested output objects retain their identity.

```{r hook-result-values, eval = TRUE}
library(deputy)
...
`continue` must be one non-missing logical value; optional text must be NULL
or one non-missing string. `suppress_output` keeps its `isTRUE()` coercion.
Errors raised while constructing results inside callbacks follow the existing
hook error path: PreToolUse fails closed; other events log and continue.
...
Set `Permissions(file_write = output_dir)` to restrict native file writes
