...
make_hooks_chat <- function() {
  docs$create_chat(list(
    ...
    docs$step_text("I inspected the fixture project successfully.")
  ))
}
```
...
```{r hooks-log-demo, eval = docs$should_eval}
chat <- make_hooks_chat()
agent <- Agent$new(
  chat = chat,
  tools = tools_file(),
...
```{r hooks-audit-demo, eval = docs$should_eval}
