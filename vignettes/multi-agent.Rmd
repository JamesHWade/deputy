...
docs <- docs_init("multi-agent", "verified-replay")
project <- docs$make_project()

setwd(project$root)
knitr::opts_knit$set(root.dir = project$root)

make_multi_agent_chat <- function() {
  chat <- docs$create_chat(list(
    docs$step_tool(
      "I'll ask the specialist to inspect the DESCRIPTION file.",
      "delegate_to_agent",
      list(
        agent_name = "code_reviewer",
        task = "Read the DESCRIPTION file and summarise what this package does."
      )
    ),
    docs$step_text(
      "The specialist reports that the package provides agentic AI workflows for R with tools, permissions, hooks, and multi-agent support."
    )
  ))
  ...
  chat$clone <- function() {
    docs$create_chat(list(
      docs$step_text(
        "The DESCRIPTION file describes deputy as an agentic AI workflow framework for R built on ellmer."
      )
    ))
  }
...
```{r multi-agent-demo, eval = docs$should_eval}
code_reviewer <- agent_definition(
  name = "code_reviewer",
