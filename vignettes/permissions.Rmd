...
docs <- docs_init("permissions", "verified-replay")
project <- docs$make_project()

setwd(project$root)
knitr::opts_knit$set(root.dir = project$root)

readonly_chat <- docs$create_chat(list(
  docs$step_tool(
    "I'll list the files with read-only permissions.",
    "list_files",
    list(path = ".")
  ),
  docs$step_text("Read-only mode lets me inspect files without modifying them.")
))
```
...
In `"readonly"` mode, only tools with `read_only_hint = TRUE` are allowed.
In `"default"` mode, tools with `destructive_hint = TRUE` may be blocked
...
```{r permissions-readonly-demo, eval = docs$should_eval}
chat <- readonly_chat
agent <- Agent$new(
  chat = chat,
  tools = tools_file(),
...
indicates what happened (`"max_turns"` or `"cost_limit"`).
